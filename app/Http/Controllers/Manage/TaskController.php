...
class TaskController extends AdminbaseController
{
    public function index() {
        ...
        return view_admin('tasks.tasks', $data);
    }
    ...
    public function load(Request $request) {
        ...
        if ($request->ajax()) {
            ...
            $discussion_projects    = Project::where('status', 0)->orderBy('order')->get();
            $todo_projects          = Project::where('status', 1)->orderBy('order')->get();
            $progress_projects      = Project::where('status', 2)->orderBy('order')->get();
            $qa_projects            = Project::where('status', 3)->orderBy('order')->get();
            $done_projects          = Project::where('status', 4)->orderBy('order')->get();
            $backlog_projects       = Project::where('status', 5)->orderBy('order')->get();
            // dd($projects);

            // $data['projects'] = $projects;
            $data['discussion_projects_html']   = view_admin('ajax.tasks.tasks', ['projects' => $discussion_projects])->render();
            $data['todo_projects_html']         = view_admin('ajax.tasks.tasks', ['projects' => $todo_projects])->render();
            $data['progress_projects_html']     = view_admin('ajax.tasks.tasks', ['projects' => $progress_projects])->render();
            $data['qa_projects_html']           = view_admin('ajax.tasks.tasks', ['projects' => $qa_projects])->render();
            $data['done_projects_html']         = view_admin('ajax.tasks.tasks', ['projects' => $done_projects])->render();
            $data['backlog_projects_html']      = view_admin('ajax.tasks.tasks', ['projects' => $backlog_projects])->render();
        ...
        if ($request->ajax()) {
            ...
            // if validation success
                ...
                if ($request->hasFile('projectfile')) {
                    ...
                    $projectmessage->project_id        = $project->project_id;
                    $projectmessage->from_id           = $fromid;
                    $projectmessage->message           = null;
                    $projectmessage->readstatus        = 0;
                    $projectmessage->status            = 1;

                    $projectmessageresult = $projectmessage->save();

                    if($projectmessageresult) {
                        ...
                        $projectmediaresult = $projectmedia->save();

                        $captionerror = $captionsuccess.'But unable save file uploaded.';
                        ...
                        if($projectmediaresult) {
                            ...
                            if($request->file('projectfile')->isValid()) {
                                ...
                                if($filecreated) {
                                    if($request->file('projectfile')->move($filepath, $basename)) {
    ...
    public function statusupdate(Request $request) {
        ...
        if ($request->ajax()) {
            ...
            if(!empty($project)) {
                ...
                if($project->update()) {
                    $data['type'] = 'success';
                    $data['caption'] = 'Status updated successfully.';
                }
                else {
                    $data['type'] = 'error';
                    $data['caption'] = 'Unable to update status.';
                }
            
            }
            else {
                $data['type'] = 'error';
                $data['caption'] = 'Invalid project.';
            }
    ...
    public function orderupdate(Request $request) {
        ...
        if ($request->ajax()) {
            ...
            $data['type'] = 'success';
            $data['caption'] = 'Ordering updated successfully.';
